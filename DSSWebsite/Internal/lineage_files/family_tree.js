...
!(function () {
    ...
    var margins = [60, 10, 60, 10],
        width_canvas = isMobile ? 1500 - margins[1] - margins[3] : screen.width - margins[1] - margins[3],
        height_canvas = 2500 - margins[0] - margins[2],
        i = 0,
        root;

    var tree = d3.layout.tree()
        .size([width_canvas, height_canvas]);

    tree.separation(function (a,b) { return 0.6; });

    var diagonal = d3.svg.diagonal()
        .projection(function(d) { return [d.x, d.y]; });

    var vis = d3.select("#family_tree").append("svg:svg")
        .attr("width", width_canvas)
        .attr("height", height_canvas + margins[0] + margins[2])
      .append("svg:g")
        .attr("transform", "translate(" + margins[3] + "," + margins[0] + ")");

    function makeTree() {
      root = gon.family_trees
      root.x0 = width_canvas / 2;
      root.y0 = 0;
    ...
    function update(source) {
      ...
      var nodeEnter = node.enter().append("svg:g")
          ...
          .on("mouseout", function(){ 
              d3.select(this).select('circle')
                  .style("fill", function(d) { return d._children ? dssBlue : white; });
          });

      // node starts at smaller size. is inflated on nodeUpdate
      var bubbles = nodeEnter.append("svg:circle")
          .attr("r", 2)
          .style("fill", dssBlue)

      // Deals with first name
      ...
      nodeUpdate.select("circle")
          .attr("r", 30)
          .style("stroke", "#fdbf5b")
          .style("fill", function(d) { return d._children ? dssBlue : "#fff"; })
...
function editImageTag(checkurl, size, gravatar_id, gravatar_hover_id, id) {
	...
	xmlhttp.onload = function() {
		if (xmlhttp.status != 404) {
			...
			elem.setAttribute("onmouseover", "$(this).attr('src','https://secure.gravatar.com/avatar/' + gravatar_id + '.png?s=' + size)")
			elem.setAttribute("onmouseout", "$(this).attr('src','https://secure.gravatar.com/avatar/' + gravatar_hover_id + '.png?s=' + size)")	
		}
	};
	xmlhttp.send();
};
...
$(document).ready(function () {
    ...
    var show_or_hide_date_select = function () {
        console.log("here")
        var is_checked = $("#show_expired_at")[0].checked ;
        if (is_checked) {
            $("#expired_at").show();
        } else {
            $("#expired_at").hide();
    ...
    $("#show_expired_at").click(show_or_hide_date_select);
});
$(document).ready(function () {

    // Triggers when clicking +x button
    $('.vote[type="radio"]').parent().click(function (e) {
        var form = $(this).closest("form");
        form.children(".btn-group").children("label").addClass("disabled");

        // Set Timeout so the default click behavior 
        // triggers before we submit the form.
        // Yayyyyy javascript call stack.
        setTimeout(function () {
            form.trigger("submit.rails");
        }, 0);
    });

    $('.vote-form').bind('ajax:success', function (e, data, status, xhr) {
        $(this).children(".btn-group").children("label").removeClass("disabled");
    });


    $('#rushee_heard_about_tt').change(function (e) {
        var hidden_input = $('#heard_tt_other');
        console.log(hidden_input)
        if (this.value == "Other") {
            $(this).parent().parent().removeClass("col-md-12").addClass("col-md-3");
            hidden_input.parent().addClass("col-md-9");
            hidden_input[0].type = "text" // [0] to get DOM node
            hidden_input.show();
        } else {
            $(this).parent().parent().removeClass("col-md-3").addClass("col-md-12");
            hidden_input.parent().removeClass("col-md-9");
            hidden_input.hide();
        }
    });

    $('#heard_tt_other').change(function (e) {
        var other_option = $('#other-option');
        other_option[0].value = this.value;
    });
});
$(document).ready(function(){
...
// Subtract 100 due to height of top navbar;
/*
  SortTable
  version 2
  7th April 2007
